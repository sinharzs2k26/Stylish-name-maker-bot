# 🤖 Stylish Name Maker Bot - Telegram Bot

A feature-rich Telegram bot that instantly transforms names into 2000+ stylish fonts, decorative art, and unique text styles.

Bot link: t.me/Style_Craftbot

## ✨ Features

- **2000+ Styles/Fonts/Art** - Massive collection of decorative designs
- **Fast Pagination** - Quick navigation through styles
- **Four Categories**:
  - 🎨 Decorative (1000+ styles)
  - 🔤 Fonts (50+ font families)
  - 🖼️ Art (500+ artistic designs)
  - 🎭 Mixed (500+ combined styles)
- **Random Name Generator** - Get random stylish names instantly
- **Bot Statistics** - Track usage and performance

## 🚀 Commands

- `/start` - Start the bot and enter your name
- `/random` - Generate a random stylish name
- `/stats` - View bot statistics (admin only)
- `/help` - Get help and instructions

## 📱 How to Use

1. Start the bot with `/start`
2. Enter your name when prompted
3. Choose from four categories:
   - Decorative
   - Fonts
   - Art
   - Mixed
4. Browse through styles with pagination
5. Copy and paste anywhere you want!